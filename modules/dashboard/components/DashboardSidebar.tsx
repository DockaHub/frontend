...
import {
    ChevronDown, Mail, MessageSquare, Users, FileInput, LayoutTemplate, FolderOpen,
    PanelLeftClose, PanelLeftOpen, X, Building2, Palette, Smile, Bell, User as UserIcon, Settings, Sun, Moon, LogOut
} from 'lucide-react';
...
interface DashboardSidebarProps {
    ...
    className?: string; // Allow custom classes for mobile positioning
...
const DashboardSidebar: React.FC<DashboardSidebarProps> = ({
    ...
    // Use organizations passed via props
    ...
    return (
        <div className={`flex flex-col bg-docka-50 dark:bg-zinc-900 pt-4 h-full border-r border-docka-200/50 dark:border-zinc-800 shrink-0 transition-all duration-300 ease-in-out relative group/sidebar ${isCollapsed ? 'w-[60px]' : 'w-[260px]'} ${className}`}>
            ...
            {/* MAIN CONTENT: Current Org Menu */}
            <div className="flex-1 overflow-y-auto custom-scrollbar px-2 pb-4 relative">

                {/* Current Org Header & Menu */}
                {/* Current Org Header & Menu */}
                <div className="mb-6 animate-in fade-in slide-in-from-left-4 duration-500">
                    {/* Current Org Header (Dropdown Trigger) */}
                    <div className={`px-2 mb-4 relative z-20 ${isCollapsed ? 'flex justify-center' : ''}`}>
                        <button
                        ...
                        >
                            <div className={`rounded-lg flex items-center justify-center text-white font-bold shadow-sm shrink-0 ${currentOrg.logoColor} ${isCollapsed ? 'w-6 h-6 text-[10px]' : 'w-8 h-8 text-xs'}`}>
                            ...
                            <>
                                ...
                                <div className={`absolute top-full mt-2 bg-white dark:bg-zinc-900 rounded-xl border border-docka-200 dark:border-zinc-700 shadow-xl z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top ${isCollapsed ? 'left-14 w-60' : 'left-2 right-2'}`}>
                                    <div className="p-2 space-y-1 max-h-[300px] overflow-y-auto custom-scrollbar">
                                        ...
                                        {organizations.filter(o => o.id !== currentOrgId).map(org => (
                                            <button
                                                key={org.id}
                    ...
                    <div className="space-y-0.5">
                        {menuItems.map(item => {
                            const isSelected = currentView === item.id || (item.children?.some(child => currentView === child.id));
                    ...
                    {/* GLOBAL APPS (Navigation Out) */}
                    <div className={`mt-8 pt-4 border-t border-docka-200 dark:border-zinc-800 ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
                        ...
                        <div className="space-y-0.5 w-full">
                            {[
                                ...
                                { id: 'meet', label: 'Meet', icon: Users, path: '/meet' }, // Using Users for Meet temporary
                                { id: 'tasks', label: 'Tarefas', icon: FileInput, path: '/tasks' }, // Using FileInput for Tasks temporary
                                { id: 'calendar', label: 'Agenda', icon: LayoutTemplate, path: '/calendar' }, // Using LayoutTemplate for Calendar
                                { id: 'drive', label: 'Drive', icon: FolderOpen, path: '/drive' },
                                { id: 'people', label: 'Pessoas', icon: Users, path: '/contacts' },
                            ].map(app => {
                                const AppLink = (
                                    <a
                                        href={app.path} // Simple navigation
                                        className={`w-full flex items-center px-2 py-2 text-sm rounded-lg text-docka-600 dark:text-zinc-400 hover:bg-docka-100/50 dark:hover:bg-zinc-800/50 hover:text-docka-900 dark:hover:text-zinc-200 transition-all duration-200 group ${isCollapsed ? 'justify-center' : ''}`}
                                    >
                                        <app.icon
                                            size={20}
                                            className={`shrink-0 text-docka-400 dark:text-zinc-500 group-hover:text-docka-600 dark:group-hover:text-zinc-300 ${!isCollapsed ? 'mr-3' : ''}`}
                                        />
                ...
                {/* NOTE: Removed bottom Workspaces list as requested */}
            </div>
