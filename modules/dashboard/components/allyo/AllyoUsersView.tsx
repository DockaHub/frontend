...
const CATEGORY_CONFIG: Record<AllyoUserCategory, { label: string; group: HierarchyGroup; level: number; permission: string }> = {
    ...
    CREATIVE_ACCOUNT_MANAGER: { label: 'Creative Account Manager', group: 'Gestão', level: 3, permission: 'Gerencia clientes, projetos, responsáveis e todas as ações administrativas das tarefas.' },
    CREATIVE_QUALITY_SPECIALIST: { label: 'Creative Quality Specialist', group: 'Gestão', level: 3, permission: 'Revisa materiais e pode administrar o fluxo das tarefas e aprovações internas.' },
    CREATIVE_ACCOUNT_SUPPORT: { label: 'Creative Account Support', group: 'Gestão', level: 3, permission: 'Apoia contas, prazos, projetos e o gerenciamento operacional das tarefas.' },
    CUSTOMER_SUPPORT: { label: 'Customer Support', group: 'Gestão', level: 3, permission: 'Atende usuários e pode gerenciar projetos e tarefas da operação.' },
    SQUAD_LEADER: { label: 'Squad Leader', group: 'Gestão', level: 3, permission: 'Coordena uma equipe e administra o fluxo das entregas.' },
    ATENDIMENTO: { label: 'Atendimento', group: 'Operação', level: 2, permission: 'Acompanha clientes e pode administrar projetos, briefings, prazos e tarefas.' },
    CRIATIVO: { label: 'Criativo', group: 'Operação', level: 2, permission: 'Executa as tarefas atribuídas, sem acesso às ações administrativas.' },
    CRIATIVO_SMB: { label: 'Criativo SMB', group: 'Operação', level: 2, permission: 'Executa tarefas da operação SMB, sem acesso às ações administrativas.' },
    ART_DIRECTOR: { label: 'Art Director', group: 'Operação', level: 2, permission: 'Executa e orienta entregas de direção de arte, sem alterar a estrutura do projeto.' },
