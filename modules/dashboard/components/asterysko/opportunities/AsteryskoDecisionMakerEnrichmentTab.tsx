...
export const AsteryskoDecisionMakerEnrichmentTab: React.FC = () => {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const [attemptResponse, countResponse] = await Promise.all([
                ...
                api.get<DecisionMakerCounts>('/asterysko/decision-maker-enrichments/counts'),
            ]);
    ...
    return (
        <div className="space-y-5">
            ...
            <DecisionMakerDetailsModal attemptId={selectedId} onClose={() => setSelectedId(null)} onChanged={load} />
        </div>
    );
};

const DecisionMakerDetailsModal: React.FC<{ attemptId: string | null; onClose: () => void; onChanged: () => Promise<void> }> = ({ attemptId, onClose, onChanged }) => {
    ...
    const load = useCallback(async () => {
        ...
        setError(null);
        try {
            setAttempt((await api.get<DecisionMakerAttempt>(`/asterysko/decision-maker-enrichments/${attemptId}`)).data);
        } catch (loadError: unknown) {
    ...
    const reject = (candidate: DecisionMakerCandidate) => {
        ...
        if (reason) void execute(`reject-${candidate.id}`, () => api.post(`/asterysko/decision-maker-enrichments/${attemptId}/candidates/${candidate.id}/reject`, { reason }));
    };
                    ...
                    {loading ? <Loader2 className="mx-auto my-16 animate-spin text-violet-600" /> : error ? <p className="text-sm text-rose-600">{error}</p> : attempt && (
                        <div className="space-y-5">
                                    ...
                                    {(attempt.candidates ?? []).length === 0 ? <p className="rounded-xl border border-dashed p-6 text-center text-xs text-zinc-400">Nenhum candidato técnico.</p> : attempt.candidates?.map(candidate => (
                                        <div key={candidate.id} className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
                                            ...
                                            <div className="mt-3 flex flex-wrap gap-2">
                                                <button disabled={!!action} onClick={() => void execute(`accept-${candidate.id}`, () => api.post(`/asterysko/decision-maker-enrichments/${attemptId}/candidates/${candidate.id}/accept`))} className="inline-flex items-center gap-1 rounded-lg bg-emerald-50 px-2.5 py-1.5 text-xs font-semibold text-emerald-700"><Check size={12} /> Aceitar</button>
                            ...
                            <div className="flex flex-wrap gap-2 border-t border-zinc-200 pt-4 dark:border-zinc-800">
                                <button disabled={!!action} onClick={() => void execute('reprocess', () => api.post(`/asterysko/decision-maker-enrichments/${attemptId}/reprocess`))} className="inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-semibold"><RefreshCw size={13} /> Reprocessar</button>
                                <button disabled={!!action} onClick={() => void execute('lock', () => api.post(`/asterysko/decision-maker-enrichments/${attemptId}/${attempt.opportunity?.decisionMakerEnrichmentLocked ? 'unlock' : 'lock'}`))} className="inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs font-semibold">{attempt.opportunity?.decisionMakerEnrichmentLocked ? <Unlock size={13} /> : <Lock size={13} />}{attempt.opportunity?.decisionMakerEnrichmentLocked ? 'Desbloquear' : 'Bloquear'}</button>
