...
import { FauvesPageHeader, Panel, SecondaryButton } from './FauvesUI';
...
const FauvesAuditSettingsView: React.FC = () => {
    ...
    return <div className="h-full min-h-0 overflow-y-auto bg-white animate-in fade-in duration-300 dark:bg-zinc-950">
        <FauvesPageHeader title="Configurações" description="Saúde e segurança da conexão administrativa entre ManySpace e Fauves." actions={<SecondaryButton onClick={() => void checkConnection()} disabled={status === 'loading'}><RefreshCw size={14} className={status === 'loading' ? 'animate-spin' : ''} /> Testar conexão</SecondaryButton>} />
        <main className="grid gap-4 p-4 sm:p-6 lg:grid-cols-[1.2fr_.8fr] lg:gap-6 lg:p-8">
            <Panel className="p-6">
